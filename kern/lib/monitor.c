...
typedef unsigned int       uintdata_t;
...
static struct Command commands[] = {
    ...
    {"backtrace", "Display the stack about the kernel", mon_backtrace},
...
int mon_backtrace(int argc, char **argv, struct Trapframe *tf)
{
    ...
    uintdata_t *p_ebp = (uintdata_t *) ebp;
    while(ebp != 0)
    {
        dprintf("  ebp %08x", ebp);
        dprintf("  eip %08x", *(p_ebp+1));
        dprintf("  args");
        for(int i=2;i<7;i++)
        {
            dprintf(" %08x", *(p_ebp+i));
