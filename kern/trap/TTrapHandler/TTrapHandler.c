...
void pgflt_handler(void)
{
    ...
    if ((errno & 0x3) == 0x3) {
        ...
        if (pte_entry & PTE_COW) {
            ...
            // TODO
            //dprintf("prepare to enter cow, errno: %08x\n", errno);
...
void exception_handler(void)
{
    ...
    {
        //dprintf("enter exception_handler.\n");
...
void interrupt_handler(void)
{
    ...
    cur_pid = get_curid();
    if(uctx_pool[cur_pid].trapno == T_IRQ0 + IRQ_TIMER)
    {
        timer_intr_handler();
    }
    else if(uctx_pool[cur_pid].trapno == T_IRQ0 + IRQ_SPURIOUS)
    {
